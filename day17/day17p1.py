...
with open("day17_input") as f:
    ...
    for x in range(0,100):
        for y in range(-600, 600):
            result, y_marks = land_in_target((x,y), x_value, y_value)
            if result == True:
                good_landing.append((x,y))
    #print(good_landing)
    highest = sorted(good_landing, key=lambda x:x[1], reverse=True)[0]
    result, y_marks = land_in_target(highest, x_value, y_value)
    print(max(y_marks))
